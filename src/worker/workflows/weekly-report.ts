...
interface TransferMetrics { count:number; }
...
export class WeeklyReportWorkflow extends WorkflowEntrypoint<Env,Params>{
  async run(event:WorkflowEvent<Params>,step:WorkflowStep){
    ...
    const metrics=await step.do('compute objective weekly metrics',async()=>{
      ...
      const errors=await this.env.DB.prepare(`SELECT category,count_30d FROM error_patterns ORDER BY count_30d DESC LIMIT 3`).all<ErrorMetric>();
      const transfer=await this.env.DB.prepare(`SELECT COUNT(*) count FROM missions WHERE status='used' AND completed_at>=? AND completed_at<?`).bind(weekStart,weekEnd).first<TransferMetrics>();
