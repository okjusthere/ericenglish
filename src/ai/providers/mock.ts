...
export class MockTextProvider implements TextModelProvider {
  ...
  async generateStructured<T>(input: TextRequest, schema: ZodType<T>): Promise<T> {
    ...
    else if(input.taskType==='event_prep') value={mustUseTerms:['asking rent','common area maintenance','What is the next step?'],recommendedOpening:'Hi, I am calling about tomorrow’s listing-agent conversation and want to confirm the commercial terms before scheduling the next step.',likelyFollowUps:['Could you confirm whether CAM is included?','Is there any flexibility on the asking rent?','When can you confirm the showing time?'],possibleResponses:['CAM is quoted separately from base rent.','The owner needs to approve any rent change.'],cheatSheet:['State the exact event purpose','Confirm rent and additional charges','Resolve one ambiguous answer','Agree on the next step'],followUpMessage:'Thank you for discussing the property. I am following up to confirm the rent, CAM treatment, and agreed next step.'};
    else {const targets=input.prompt.match(/Target units:\s*([^\n]+)/)?.[1]?.split(',').map((item)=>item.trim()).filter(Boolean)??['clarify'];value = { taskCompletion:4,clarity:4,grammar:3,lexicalRange:3,naturalness:3,pragmatics:4,fluency:3,summary:'The objective was clear and the next step was confirmed.',priorityCorrections:corrections,successfulTargetUnits:targets.slice(0,1),missedTargetUnits:targets.slice(1,2),newCandidateUnits:['room for negotiation'] };}
