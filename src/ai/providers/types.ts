...
export interface TextResult { text: string; provider: string; model: string; latencyMs: number; inputTokens?: number; outputTokens?: number; requestId?: string; }
...
export interface TranscriptResult { text: string; confidence?: number; durationSeconds?: number; provider: string; model: string; latencyMs?: number; requestId?: string; }
export interface SpeechToTextProvider { transcribe(audio: ArrayBuffer, mimeType: string): Promise<TranscriptResult>; }
export interface AudioResult { audio: ArrayBuffer; contentType: string; provider: string; model: string; latencyMs?: number; requestId?: string; }
